...
# 내부 조인 예제 1
# : 구매 테이블에서 momber_id가 1인 사람이 구매한 물건을 확인
select * 
from purchases inner join members on purchases.member_id = members.member_id
...
from members as M inner join purchases as P on M.member_id = P.member_id
where P.amount >= 20000;

# 내부 조인 예제 3
# 회원 이름별로 구매 금액을 내림차순 정렬
select M.name, sum(P.amount) as total_purchase
from members as M
