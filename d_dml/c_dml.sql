...
/*
	...
	-- 열 이름을 생략할 경우 values 키워드 뒤 값들의 순서는 테이블 정의 시 지정한 열의 순서 & 개수와 동일
...
# 2) 컬럼명 지정 O
insert into `example01` 
	values ('최윤성', '전공 B', 28);		-- name, age, major 타입 순서대로 삽입을 하지않아 오류가 난다. 
    
# cf) 데이터 삽입 시 데이터 값 입력을 하지 않은 경우
# 1, Null 허용 - 자동 null 지정
# 2,. NOT NULL 지정 - 오류 
...
insert into `example03`
	select * from `example02`;
-- >> 데이터를 받을 테이블과 전달할 데이터의 컬럼 비교가 필수
