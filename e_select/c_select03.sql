...
-- 오름차순: 알파벳(a), 자음(ㄱ), 날짜(오래된 순), 숫자(작은 데이터 부터)
...
# 4. group by
# : 그룹으로 묶어주는 역할
# >> 여러 행을 구룹화하여 집계함수를 적용에 주로 사용

# cf) 집계함수 
# : 그룹화 된 데이터에 대한 계산

# sum(): 합계, avg(): 평균, min(): 최소값, max(): 최대값 
# count(): 행의 개수 
...
-- 지역별 평균 포인트 계산
select area_code, avg(points) member_point 
from members
group by area_code;
...
from members
group by area_code having avg(points) >= 200;
